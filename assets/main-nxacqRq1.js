(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const t of s)if(t.type==="childList")for(const o of t.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function a(s){const t={};return s.integrity&&(t.integrity=s.integrity),s.referrerPolicy&&(t.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?t.credentials="include":s.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function r(s){if(s.ep)return;s.ep=!0;const t=a(s);fetch(s.href,t)}})();class c{constructor(e=""){this.baseUrl=e}async health(){const e=await fetch(`${this.baseUrl}/api/sdk/health`);if(!e.ok)throw new Error("SDK health check failed");return e.json()}async createSession(e={}){return{ok:!0,id:`session-${Date.now()}`,mode:e.mode||"agentic",focus:e.focus||"ship faster"}}async executePrompt(e,a={}){return{ok:!0,prompt:e,response:`Krims Code ready to assist with: ${e}`,mode:a.mode||"agentic"}}}const l="/assets/logo-D7sWxW1Z.png",n=document.getElementById("app"),d=new c;n&&(n.innerHTML=`
    <div class="wrap">
      <header class="topbar">
        <a class="brand" href="#">
          <img src="${l}" alt="Krims Code logo" />
  ...
  `);Promise.all([fetch("/api/health").then(i=>i.json()),d.health().catch(()=>({ok:!1}))]).then(([i,e])=>{var r;const a=document.createElement("div");a.className="status-pill",a.textContent=i.ok&&e.ok?"System online":"Warming up",(r=document.querySelector(".topbar"))==null||r.appendChild(a)}).catch(()=>{});
